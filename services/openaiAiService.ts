...
class OpenaiAiService implements AiService {
  ...
  async initializeWithKey(apiKey: string): Promise<boolean> {
    if (!apiKey) {
      ...
      console.warn("Attempted to initialize OpenAI with an empty API key.");
      return false;
    }
    try {
      this.openai = new OpenAI({ apiKey, dangerouslyAllowBrowser: true });
      this.currentApiKey = apiKey;
      console.log("OpenaiAiService initialized with user-provided key.");
      return true;
    } catch (error) {
      console.error("Failed to initialize OpenAI client with provided key:", error);
      this.openai = null;
      this.currentApiKey = null;
      return false;
    }
  ...
  isConfigured(): boolean {
    return this.openai !== null && this.currentApiKey !== null;
  }
...
`;
  ...
  async generateColorPalettes(inspiration: BrandInspiration, modelName: string = OPENAI_MODEL_NAME): Promise<ColorPalette[]> {
    if (!this.isConfigured() || !this.openai) {
      throw new Error("OpenAI client is not initialized. Please ensure your OpenAI API key is set in settings.");
    }
    ...
    try {
      ...
      // OpenAI's JSON mode sometimes wraps the object in an outer key, e.g. {"palettes": [...]}. Try to extract the array.
      try {
        const potentialObject = JSON.parse(jsonStr);
        if (typeof potentialObject === 'object' && potentialObject !== null) {
            const keys = Object.keys(potentialObject);
            if (keys.length === 1 && Array.isArray(potentialObject[keys[0]])) {
                jsonStr = JSON.stringify(potentialObject[keys[0]]);
            }
        }
      } catch (e) {
        // If it's not an object or parsing fails, proceed with jsonStr as is
      }
      ...
      if (!Array.isArray(parsedData) || parsedData.length !== 4 || parsedData.some(p =>
          ...
          console.error("Invalid palette structure or count received from OpenAI API:", parsedData);
          throw new Error(`OpenAI API's response was not in the expected format (expected 4 palettes with new font structure). Raw response snippet: ${responseText.substring(0, 300)}...`);
      }

      return parsedData.map(p => ({ ...p, implementationIdeas: undefined, designPrompt: undefined })) as ColorPalette[];

    } catch (error) {
      console.error("Error generating color palettes with OpenAI:", error);
      const baseMessage = `OpenAI API Error: Failed to generate palettes.`;
      if (error instanceof SyntaxError) {
          const rawText = responseText || "API response text not available";
          console.error("Malformed JSON response from OpenAI API:", rawText);
  ...
  async generateImplementationIdeas(palette: ColorPalette, inspiration: BrandInspiration, modelName: string = OPENAI_MODEL_NAME): Promise<string> {
    if (!this.isConfigured() || !this.openai) {
      throw new Error("OpenAI client is not initialized. Please ensure your OpenAI API key is set in settings.");
    }
...
`;
    ...
    try {
      ...
      if (!text) throw new Error("OpenAI API returned an empty response for implementation ideas.");
      return text.trim();
    } catch (error) {
      console.error("Error generating implementation ideas with OpenAI:", error);
      const baseMessage = "OpenAI API Error: Failed to generate implementation ideas.";
      if (error instanceof Error) {
  ...
  async generateDesignPrompt(palette: ColorPalette, inspiration: BrandInspiration, modelName: string = OPENAI_MODEL_NAME): Promise<string> {
    if (!this.isConfigured() || !this.openai) {
      throw new Error("OpenAI client is not initialized. Please ensure your OpenAI API key is set in settings.");
    }
...
`;
    ...
    try {
      ...
      const text = completion.choices[0]?.message?.content;
      if (!text) throw new Error("OpenAI API returned an empty response for design prompt.");
      return text.trim();
    } catch (error) {
      console.error("Error generating design prompt with OpenAI:", error);
      const baseMessage = "OpenAI API Error: Failed to generate design prompt.";
      if (error instanceof Error) {
        throw new Error(`${baseMessage} ${error.message}`);
      }
      throw new Error(`${baseMessage} An unknown error occurred.`);
...
export default new OpenaiAiService();